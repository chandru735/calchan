...
class BasePage:
    ...
    def keycode(self,value):
        try:
            ...
            self.log.info(f"Element with keycode value {value} has been pressed")
        except:
            self.log.info(f"Element with keycode value {value} is not pressed")
            self.takeScreenshot(value)
            assert False


    def singleTap(self,x,y):
        singleaction = TouchAction(self.driver)
        element = None
        try:
            singleaction.tap(None,x,y,1).perform()
            self.log.info(f"single tapped on the element")

        except:
            self.log.info(f"failed to single tap on the element")
            self.takeScreenshot(x)
            assert False
